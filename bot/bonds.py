...
def add_bond(user_id, purchase_price, purchase_date, quantity, nominal_value=None, maturity_date=None, dividend=None, name=None):
    try:
        ...
        c = conn.cursor()
        c.execute('''INSERT INTO portfolio (id_user, type, purchase_price, purchase_date, quantity, nominal_value, maturity_date, dividend, name) 
                     VALUES (?, 'bonds', ?, ?, ?, ?, ?, ?, ?)''',
                  (user_id, purchase_price, purchase_date, quantity, nominal_value, maturity_date, dividend, name))
        conn.commit()
        conn.close()
        return True
...
def process_bond_name_step(message, bot: TeleBot, price, date, quantity, nominal, maturity_date, dividend):
    ...
    if add_bond(message.chat.id, price, date, quantity, nominal_value=nominal, maturity_date=maturity_date, dividend=dividend, name=name):
